...
const kindLink = 'text-stone-500 hover:text-stone-900 dark:text-ink-400 dark:hover:text-ink-100';
const kindLinkOn = 'font-semibold text-stone-900 dark:text-ink-100';
const menuLink =
...
export const IndexHeader: FC<IndexHeaderProps> = ({ basePath, current, projects, view }) => (
  <header class="flex items-center gap-7 border-b border-stone-200 py-4 dark:border-ink-800">
    <a href="/" class="mr-3 text-lg font-bold tracking-tight text-stone-900 dark:text-ink-100">
      a<span class="text-amber-600 dark:text-accent">.</span> Artifacts
    </a>
    <details class="relative">
      <summary class="cursor-pointer list-none font-semibold text-stone-900 select-none dark:text-ink-100 [&::-webkit-details-marker]:hidden">
        {current ?? 'All projects'}
        <span class="ml-1 font-normal text-stone-400 dark:text-ink-400">&#9662;</span>
      </summary>
      <div class="absolute top-9 -left-3 z-10 w-[600px] columns-3 gap-x-8 border border-stone-200 bg-white px-4 py-3 shadow-lg dark:border-ink-800 dark:bg-ink-900 dark:shadow-black/50">
    ...
    <nav class="ml-auto flex items-baseline gap-5">
    ...
    <span class="border-l border-stone-200 pl-5 text-[13px] text-stone-500 tabular-nums dark:border-ink-800 dark:text-ink-400">
